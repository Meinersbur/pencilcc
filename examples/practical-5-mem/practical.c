...
int main () {
	float (*gauss)[FILTER_HEIGHT] = prl_alloc(FILTER_WIDTH * sizeof*gauss);
	float (*waves)[IMAGE_WIDTH][IMAGE_HEIGHT] = prl_alloc(N_IMAGES * sizeof*waves);
	float (*blurred)[IMAGE_WIDTH][IMAGE_HEIGHT] = prl_alloc(N_IMAGES * sizeof*blurred);
	int32_t (*hist)[HISTOGRAM_STEPS] = prl_alloc(N_IMAGES * sizeof*hist);
