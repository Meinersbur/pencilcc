...
int main () {
	...
	for (int i = 0; i < 5; i += 1) {
		gen_waves(N_IMAGES, IMAGE_WIDTH, IMAGE_HEIGHT, waves);
		convolution(N_IMAGES, IMAGE_WIDTH, IMAGE_HEIGHT, waves, FILTER_WIDTH, FILTER_HEIGHT, gauss, blurred);
		histogram(N_IMAGES, IMAGE_WIDTH, IMAGE_HEIGHT, FILTER_WIDTH, FILTER_HEIGHT, blurred, hist);
		print_histogram(N_IMAGES, hist);
	}
